...
(function($) {
	const commonLibraryUrl = '/statics/lib/fancy-file-uploader'
	document.write(`<script src="${commonLibraryUrl}/jquery.ui.widget.js"></script>`)
	document.write(`<script src="${commonLibraryUrl}/jquery.fileupload.js"></script>`)
	document.write(`<script src="${commonLibraryUrl}/jquery.iframe-transport.js"></script>`)
	...
	$.fn.FancyFileUpload = function(options) {
		...
		var AddFile = function(uploads, e, data) {
			...
			if (settings.maxfilesize > -1 && data.files[0].size > settings.maxfilesize)  data.ff_info.errors.push(FormatStr(Translate('{0} 이내 파일을 첨부해주세요.'), GetDisplayFilesize(settings.maxfilesize, settings.adjustprecision, settings.displayunits)));
			...
			dropzone.html(`
				<div class="flex--column flex-align--center">
					...
					<button type="button" class="button button--primary">
						<img src="/statics/icon/close-line.svg" class="icon" alt="파일업로드">
						파일 업로드
					</button>
				</div>
